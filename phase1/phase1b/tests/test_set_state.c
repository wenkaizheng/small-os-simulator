...
// Tests setting the state of Parent to JOINING when it has a Child that already quit
...
Output(void *arg) 
{
    ...
    USLOSS_Console("Parent Hello now forks a Child with priority 1\n");
    val = P1_Fork("Child", Child, (void *) 0, USLOSS_MIN_STACK, 1, 0, &child);
    assert(val == P1_SUCCESS);
    
    USLOSS_Console("Setting Parent state to P1_STATE_JOINING\n");
    // set Hello to P1_STATE_JOINING but not possible since Child has quit
    val = P1SetState(1, P1_STATE_JOINING, 0);
    if(val != P1_CHILD_QUIT){
 	USLOSS_Console("TEST FAILED\n");
	USLOSS_Halt(0);
    }
    
    USLOSS_Console("Return value is P1_CHILD_QUIT\n");

    // check to make sure Hello's state is not P1_STATE_JOINING
    P1_ProcInfo *info = malloc(sizeof(P1_ProcInfo));
    val = P1_GetProcInfo(1, info);
    assert(val == P1_SUCCESS);
    // This is actually checking that it's RUNNING, not what the comment says above.
    if(info->state != P1_STATE_RUNNING){
	USLOSS_Console("TEST FAILED\n");
        USLOSS_Halt(0);
    }
    free(info);
    USLOSS_Console("Parent's state is P1_STATE_RUNNING\n");
