...
Reader(void *arg)
{
   ...
   // int i;
    ...
    rc = Sys_Spawn("Reader1", Reader1, (void *) sid, USLOSS_MIN_STACK * 2, 3, &pid);
    assert(rc == P1_SUCCESS);

    rc = Sys_Spawn("Writer1", Writer1, (void *) sid, USLOSS_MIN_STACK * 2, 3, &pid);
    assert(rc == P1_SUCCESS);
   
    for (int i = 0; i < 2; i++) {
        rc = Sys_Wait(&pid, &child);
        USLOSS_Console("152th\n");
        assert(rc == P1_SUCCESS);
    }
    TEST(P3_vmStats.faults, 0);
    Sys_VmShutdown();
