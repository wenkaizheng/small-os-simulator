...
Child(void *arg)
{
    ...
    int status;
    if(num %3 ==0){
        sprintf(buffer, "P3C_PAGERS_%d", num);
        rc = Sys_Spawn(buffer, Child1, NULL, USLOSS_MIN_STACK * 4,random()%4+1, &pid);
...
P4_Startup(void *arg)
{
    ...
    rc = Sys_VmInit(PAGES, PAGES, (numChildren+count) * PAGES, PAGERS, (void **) &vmRegion);
    assert(rc== P1_SUCCESS);
