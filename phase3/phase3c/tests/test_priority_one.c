...
Child1(void *arg)
{
    ...
    for (j = 0; j < PAGES; j++) {
        ...
        Debug("73 Child \"%s\" reading zeros from page %d @ %p\n", name, j, page);
...
Child(void *arg)
{
    ...
    for (i = 0; i < numChildren; i++) {
        rc = Sys_Spawn(names[i], Child1, (void *) names[i], USLOSS_MIN_STACK * 4, 1, &pid);
        assert(rc == P1_SUCCESS);
    }
    for (i = 0; i < numChildren; i++) {
    ...
    for (i = 0; i < ITERATIONS; i++) {
        for (j = PAGES-1; j > 0; j--) {
            ...
            assert(rc == P1_SUCCESS);
            page = vmRegion + j * pageSize;
            Debug("132 Child \"%s\" writing to page %d @ %p\n", name, j, page);
            for (int k = pageSize-1; k >0; k--) {
                page[k] = *name+n%7;
            }
            Debug("136 Child \"%s\" reading from page %d @ %p\n", name, j, page);
            for (int k = pageSize-1; k >0; k--) {
                assert(page[k]== *name+n%7);
            }
        }
...
P4_Startup(void *arg)
{
    ...
    rc = Sys_Spawn("child_no1", Child, (void *)"a", USLOSS_MIN_STACK * 4, 1, &pid);
    assert(rc == P1_SUCCESS);
    rc = Sys_Wait(&pid, &status);
    assert(rc == P1_SUCCESS);
    assert(status== 0);
