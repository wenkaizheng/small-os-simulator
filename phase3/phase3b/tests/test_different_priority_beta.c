...
static void (*clock)(int, void *);

void Clock(int type, void *arg) 
{
    USLOSS_Console("tick\n");
    (*clock)(type, arg);
}
...
int P4_Startup(void *arg)
{
    ...
    PID child;
    SID sid;
      clock = USLOSS_IntVec[USLOSS_CLOCK_INT];
      USLOSS_IntVec[USLOSS_CLOCK_INT] = Clock;
    ...
    for (int i = 0; i < 15;i++)
    {
        ...
        rc = Sys_Spawn(buffer, Reader, (void *)sid, USLOSS_MIN_STACK * 2, 5, &pid);
        assert(rc == P1_SUCCESS);
        
        sprintf(buffer, "Writer %d",i ); 

        rc = Sys_Spawn(buffer, Writer, (void *)sid, USLOSS_MIN_STACK * 2, 4, &pid);
        assert(rc == P1_SUCCESS);
    }
    for (i = 0; i < 30; i++)
    {
