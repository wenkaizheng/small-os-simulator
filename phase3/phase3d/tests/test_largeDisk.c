...
#define PAGES 8         // # of pages per process (be sure to try different values)
#define FRAMES 10
#define ITERATIONS 3
...
P4_Startup(void *arg)
{
    ...
    for (i = 0; i < numChildren; i++) {
        rc = Sys_Spawn(names[i], Child, (void *) names[i], USLOSS_MIN_STACK * 4, (names[i][0]-'A'+1)%4+1, &pid);
        assert(rc == P1_SUCCESS);
    }
    for (i = 0; i < numChildren; i++) {
        ...
        assert(rc == P1_SUCCESS);
       // TEST(status, 0);
       assert(status ==0 || status == P3_OUT_OF_SWAP );
    }
...
void test_setup(int argc, char **argv) {
    DeleteAllDisks();
    int rc = Disk_Create(NULL, P3_SWAP_DISK, numChildren*60);
    assert(rc == 0);    
}
