...
#define PAGES 3        // # of pages per process (be sure to try different values)
#define FRAMES 1
...
P4_Startup(void *arg)
{
    ...
    for (i = 0; i < numChildren; i++) {
        rc = Sys_Spawn(names[i], Child, (void *) names[i], USLOSS_MIN_STACK * 4, 1, &pid);
        assert(rc == P1_SUCCESS);
    }
    for (i = 0; i < numChildren; i++) {
